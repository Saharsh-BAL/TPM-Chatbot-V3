...
import { PrimaryButton, Checkbox, DefaultButton, Dialog, FontIcon, Stack, Text } from '@fluentui/react'
  ...
  return (
    <>
      <Stack className={styles.answerContainer} tabIndex={0}>
        ...
        {parsedAnswer?.generated_chart && (
  <Stack className={'generated_chart'}>
    <Stack.Item grow>
      <img
        src={`data:image/png;base64,${parsedAnswer.generated_chart}`}
        alt="Mermaid Chart"
      />
    </Stack.Item>
 {/* <Stack.Item>
      <PrimaryButton
        onClick={() => {
          if (!parsedAnswer?.generated_chart) {
            console.error("Error: Mermaid syntax is missing!");
            return;
          }

          try {
            // Convert the Mermaid chart to a compressed Base64 string
            const compressedData = pako.deflate(parsedAnswer.generated_chart, { level: 9 });
            console.log(compressedData);
            const base64Chart = btoa(String.fromCharCode(...compressedData));
            console.log(base64Chart);

            // Generate the Mermaid Live URL
            const safeBase64Chart = base64Chart.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
            const mermaidLiveURL = `https://mermaid.live/edit#pako:${safeBase64Chart}`;
            
            // Open in a new tab
            window.open(mermaidLiveURL, '_blank');
          } catch (error) {
            console.error("Error compressing Mermaid syntax:", error);
          }
        }}
      >
